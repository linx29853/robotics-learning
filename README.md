...
我希望每一步都能做到：解释公式、自己写代码、用另一种方法验证结果，并记录犯过的错误。当前聚焦运动学，后续逐步学习数值逆运动学、轨迹规划和控制。
...
`archive` 保留本人原始练习；整理版修正脚本入口、统一参数，并增强验证。MuJoCo 演示与文档整理使用 AI 辅助，目标是帮助我理解和复做，不能将辅助生成的验证等同于本人已独立掌握。此次提交是已有学习资料的归档，不重造此前提交时间线。
...
如果只做前三个数学实验，只需安装 NumPy。MuJoCo 交互窗口需要桌面图形环境；`--check` 无需打开窗口。
